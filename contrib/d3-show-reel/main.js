...
function show(data) {
    ...
    if (metrix === "row") {
        ...
        if (!target) {
            ...
            createTargetSelector(targets, { value: target, type: "dropdown" });
        }
        data.slice(1).forEach(function (d) {
            var tmp = {};
            tmp.symbol = d[0];
            tmp.date = e2d3.dateObjecter(d[1]);
            var priceKey = 2;
            head.map(function (dd, i) {
                if (dd === target) priceKey = i;
            });
            tmp.price = d[priceKey];
            //console.log(new Date((Date.parse(tmp.date))));
    ...
    function draw(k) {
        g.each(function (d) {
                ...
                e.selectAll("circle, text")
                    .data(function (d) { if(d) return [d.values[k], d.values[k]]; })
                    .attr("transform", function (d) { if (d) return "translate(" + x(d.date) + "," + y(d.price) + ")"; });
        });
    }
